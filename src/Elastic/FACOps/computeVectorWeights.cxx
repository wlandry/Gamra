  ...
  for (ln = finest_ln; ln >= coarsest_ln; --ln)
    {
      ...
      if (ln < finest_ln)
        {
          /// First get the boxes that describe index space of the
          /// next finer level and coarsen them to describe
          /// corresponding index space at this level.
          SAMRAI::hier::PatchLevel &next_finer_level =
            *(hierarchy.getPatchLevel(ln + 1));
          SAMRAI::hier::BoxContainer
            coarsened_boxes(next_finer_level.getBoxes());
          ...
          /// Then set vector weight to 0 wherever there is a nonempty
          /// intersection with the next finer level.  Note that all
          ...
          for (SAMRAI::hier::PatchLevel::Iterator p(level->begin());
            ...
            {
              boost::shared_ptr<SAMRAI::hier::Patch> patch = *p;
              for (SAMRAI::hier::BoxContainer::iterator
                     i=coarsened_boxes.begin();
