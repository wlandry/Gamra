 ...
 int depth, double) const
{
  if(variable_name=="Strain")
    {
      pack_strain(buffer,patch,region,depth,dimension,!faults.empty(),
                  have_embedded_boundary(),v_id,dv_diagonal_id,dv_mixed_id,
                  level_set_id);
    }
  else if(variable_name=="Level Set")
    {
      pack_level_set(buffer,patch,region,dimension,level_set_id);
    }
  else if(variable_name=="Initial Displacement")
    {
      pack_v_initial(buffer,patch,region,depth,dimension,offset_vector_on_output,
                     v_initial);
    }
  else
    {
      pack_v_v_rhs(buffer,patch,region,variable_name,depth,dimension,
